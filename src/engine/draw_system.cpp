...
draw_system::tick(registry& reg, float dt)
{
	...
	for (auto& entity : background_view) {
		...
		if (registry.all_of<draw_color_component>(entity)) {
			const auto draw_color_comp = registry.try_get<draw_color_component>(entity);
		...
		if (registry.all_of<draw_texture_component>(entity)) {
			const auto draw_texture_comp = registry.try_get<draw_texture_component>(entity);
