...
{
	ark_float_vec2 screen_center = camera::world2screen(center);
...
{
	ark_float_vec2 screen_line = camera::world2screen(p1);
	ark_float_vec2 screen_line2 = camera::world2screen(p2);
...
void DebugDraw::DrawTransform(const b2Transform& xf)
{
	...
	ark_float_vec2 p1 = camera::world2screen(xf.p), p2;
...
{
	ark_float_vec2 screen_line = camera::world2screen(p);
...
{
	ark_float_vec2 p1 = camera::world2screen(aabb->lowerBound);
	ark_float_vec2 p2 = camera::world2screen({ aabb->upperBound.x, aabb->lowerBound.y });
	ark_float_vec2 p3 = camera::world2screen(aabb->upperBound);
	ark_float_vec2 p4 = camera::world2screen({ aabb->lowerBound.x, aabb->upperBound.y });
