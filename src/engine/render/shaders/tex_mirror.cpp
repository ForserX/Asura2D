#include "pch.h"
#include "tex_mirror.h"

using namespace Asura;
using namespace Asura::Render::Shaders;

TextureMirror::TextureMirror()
{
	TextureShader = new Shaders::ShaderProgram("image_mirror_ps.glsl", "image_rotation_vs.glsl");
}
...
void TextureMirror::Render(const RenderData& Data)
{
	RenderInstance(Data.ScaleX, Data.ScaleY, Data.x, Data.y, Data.Angle, Data.TextureID);
}

void TextureMirror::RenderInstance(float ScaleX, float ScaleY, float PosX, float PosY, float Angle, GLuint Texture)
{
	glEnable(GL_BLEND);
